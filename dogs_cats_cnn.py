...
#Extras if you want to use the model to run predictions on your own Test set
...
#Predict with model
predict = new_model.predict(test_generator, steps=np.ceil(nb_samples/batch_size))
predict

#Get the prediction from the array and Round to the nearest integer since its Binary Classification (1 or 0)
result = [int(round(p[0])) for p in predict]
result
